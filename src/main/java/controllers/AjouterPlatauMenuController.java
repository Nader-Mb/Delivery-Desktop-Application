...
public class AjouterPlatauMenuController implements Initializable {
    @FXML
    private Button btAjPlat;
    @FXML
    ...
    public AjouterPlatauMenuController() {
    }
    ...
    public void AjouterPlat(ActionEvent event) {
        Alert alert;
            if (this.tfNameD.getText().isEmpty()|| this.tfPriceD.getText().isEmpty()||this.CombCategoryD.getValue().isEmpty() ||this.tfIngD.getText().isEmpty()||this.tfURL_ImgD.getText().isEmpty()){alert = new Alert(Alert.AlertType.WARNING);
        ...
        try {
            this.MenuCrud.ajouter(new Menu(this.tfNameD.getText(), Float.parseFloat(this.tfPriceD.getText()),this.CombCategoryD.getValue() ,this.tfIngD.getText(), 2,this.tfURL_ImgD.getText()));
            //INSERT INTO `menu`(`nameP`,`priceP`,`categoryP`,`ingredientsP`,`restaurantId`,`imageP`)
        ...
        try {
            ...
            btAjPlat.getScene().setRoot(root);
        } catch (IOException e) {
