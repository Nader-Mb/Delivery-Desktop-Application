...
public class AddCampaignController implements Initializable {
    ...
    @FXML
    private Button addCampainButton;
    ...
    private final CampaignService cs = new CampaignService();
    ...
    @FXML
    void addCampaign(ActionEvent event) {
        if (numberFiled.getText().isEmpty() || GoalField.getText().isEmpty() || campaignTitleField.getText().isEmpty() || associationNameField.getText().isEmpty() ||
            ...
            try {
                cs.ajouter(new Campaign(Integer.parseInt(numberFiled.getText()), Integer.parseInt(GoalField.getText()),
                        campaignTitleField.getText(), associationNameField.getText(), campaignTypeField.getValue(), descriptionField.getText(), 0));
                switchToDisplayAllCampaigns();
            } catch (SQLException e) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Error while adding a campaign");
                alert.setContentText(e.getMessage());
                alert.showAndWait();
            }
        }
